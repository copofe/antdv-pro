...
import { useTableQuery } from '::/view/composables/table-query'
import type { CrudTableModel } from '::/repositories/list'
import { listRepo } from '::/repositories/list'
...
const { state, initQuery, resetQuery, query } = useTableQuery({
  queryApi: params => listRepo.getCurdTableList(params),
  queryParams: {
    name: undefined,
    value: undefined,
    remark: undefined,
...
async function handleDelete(record: CrudTableModel) {
  ...
  try {
    const { data } = await listRepo.delCurdTableItem(record.id)
    if (data.code === 200)
      await query()
