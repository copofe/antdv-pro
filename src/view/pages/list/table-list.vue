...
import type { ConsultTableModel, ConsultTableParams } from '::/repositories/list'
...
async function init() {
  ...
  loading.value = true
  try {
    const data = await listRepo.getTableList({
      ...formModel,
...
async function handleDelete(record: ConsultTableModel) {
  ...
  try {
    const { data } = await listRepo.delTableItem(record!.id)
    if (data.code === 200)
      await init()
