...
import router from '.'
import { isUrl } from '::/view/utils/tools'
import { DYNAMIC_LOAD_WAY, DynamicLoadEnum, type MenuDataItem } from '::/entities/app.model'
import dynamicRoutes from '::/view/router/dynamic-routes'
import { ROOT_ROUTE_REDIRECT_PATH, rootRoute } from '::/view/router/constant'
import { i18n } from '::/view/plugins/i18n'
import { userRepo } from '::/repositories/user'

async function getMenuRoutes() {
  const data = await userRepo.getUserMenus()
...
export async function generateDynamicRoutes() {
  ...
  return { menuData: treeMenuData, routerData }
}
 ...
 */
export function generateTreeRoutes(menus: MenuDataItem[]) {
